package com.velocityPort.storage.sqlite;
...
public class SQLite implements Storage {
   ...
   public SQLite(DatabaseConnector var1) {
      this.limit = 1;
   ...
   private PlayerProfile loadData(String var1, UUID var2, boolean var3) {
      ...
      if (var4 != null) {
         ...
         if (!var3) {
            ...
            this.database.update("uf_users").set(var8.toString(), var8.select(var1, var2)).set("lastSeen", System.currentTimeMillis()).where(this.lookup.toString(), this.lookup.select(var1, var2)).execute();
         }
   ...
   public boolean removeFriend(PlayerProfile var1, PlayerID var2) {
      ...
      int var3 = Math.min(var1.getId(), var2.getId());
      int var4 = Math.max(var1.getId(), var2.getId());
      return var3 == var4 ? false : this.database.delete("uf_user_friends").where("user_id_1", var3).andWhere("user_id_2", var4).execute();
   }

   public boolean prepareDB() {
      if (!this.database.sql("CREATE TABLE IF NOT EXISTS uf_users (  id   integer PRIMARY KEY AUTOINCREMENT,  name varchar(16) NOT NULL,  uuid CHAR(36)    NOT NULL)").execute()) {
         // It might fail or it might succeed if table exists. The SQLite JDBC usually returns false for execute() if no result set.
      }
      
      // Indexes should also use IF NOT EXISTS if possible in SQLite, or we can just try/catch them or check if they exist.
      // Easiest is to just CREATE INDEX IF NOT EXISTS in SQLite >= 3.3.0
      this.database.sql("CREATE INDEX IF NOT EXISTS uf_users_name_index ON uf_users (name)").execute();
      this.database.sql("CREATE UNIQUE INDEX IF NOT EXISTS uf_users_uuid_uindex ON uf_users (uuid)").execute();

      if (!this.database.sql("CREATE TABLE IF NOT EXISTS uf_user_friends (  user_id_1 int NOT NULL,  user_id_2 int NOT NULL,  CONSTRAINT uf_user_friends_user_1_user_2_pk PRIMARY KEY (user_id_1, user_id_2),  CONSTRAINT uf_user_friends_uf_users_id_fk FOREIGN KEY (user_id_1) REFERENCES uf_users (id) ON DELETE CASCADE,  CONSTRAINT uf_user_friends_uf_users_id_fk_2 FOREIGN KEY (user_id_2) REFERENCES uf_users (id) ON DELETE CASCADE)").execute()) {
         // ignore
      }

      this.database.sql("CREATE INDEX IF NOT EXISTS uf_user_friends_user_id_2_index ON uf_user_friends (user_id_2)").execute();
   ...
   private static void $$$reportNull$$$0(int var0) {
      ...
      var10001[1] = "com/velocityPort/storage/sqlite/SQLite";
      switch(var0) {
      case 0:
      default:
         var10001[2] = "<init>";
